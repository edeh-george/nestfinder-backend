    ...
    def get_image_url_list(self, obj):
        ...
        if request:
            image_urls = [
                ...
                for image in obj.image_list
                if image.images
            ]
            if os.getenv("DEBUG"):
                image_urls = [
                    image.replace("http://", "https://") for image in image_urls
                ]
            return image_urls

        image_urls = [image.images.url for image in obj.image_list if image.image]

    def get_related_apartments(self, obj):
        apartment_ids = [str(tag.id) for tag in obj.related_apartment]
        related_apartment = [
            ApartmentSerializer(Apartment.objects.get(pk=int(id))).data
