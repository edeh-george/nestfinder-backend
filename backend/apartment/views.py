...
from .serializers import ApartmentDetailSerializer, ApartmentSerializer
...
class ApartmentFilter(django_filters.FilterSet):
    ...
    apartment_type = django_filters.CharFilter(
        field_name="apartment_type", lookup_expr="icontains"
    )
...
class BulkCreateApartmentView(generics.GenericAPIView):
    ...
    def get(self, request, *args, **kwargs):
        ...
        if not os.path.exists(file_path):
            return Response(
                {"error": "JSON file not found"}, status=status.HTTP_400_BAD_REQUEST
            )
        ...
        try:
            ...
            created_apartments = Apartment.objects.filter(
                id__in=[a.id for a in apartments]
            )
        ...
        return Response(
            {"message": f"{len(apartments)} Apartments created successfully!"},
...
class AddRelatedApartment(generics.GenericAPIView):
    ...
    def get(self, request, *args, **kwargs):
        ...
        for apartment in Apartment.objects.all():
            related_apartments = (
                Apartment.objects.filter(location=apartment.location)
        ...
        return Response(
            {"message": "Successfully added related houses"}, status=status.HTTP_200_OK
        )
