...
def user_directory_path(instance, filename):
    user_id = (
        ...
        if isinstance(instance, Apartment)
        else instance.apartment.uploaded_by.id
    )
    apartment_id = (
        instance.id if isinstance(instance, Apartment) else instance.apartment.id
    )
...
class Apartment(UUidModelAbstract, models.Model):
    ...
    uploaded_by = models.ForeignKey(
        UserModel, on_delete=models.CASCADE, related_name="apartments"
    )

    name = models.CharField(max_length=255, null=True, blank=True)
    apartment_type = models.CharField(
        max_length=50, choices=APARTMENT_LIST, default="one_room"
    )
    description = models.TextField()
    price = models.PositiveIntegerField()
    location = models.CharField(max_length=255, choices=LOCATION, default="HT")
