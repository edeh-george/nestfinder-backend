...
from utils.send_mail import decrypt_token, send_email, verify_token
    ...
    def post(self, request: Request, *args, **kwargs):
        try:
            ...
            response = requests.post(
                url=f"https://{request.get_host()}/{os.getenv("API_VERSION")}mail/verify-token/",
                params={"safe": data["safe"], "token": data["token"]},
                verify=False,
            )
            ...
            if isinstance(user, User):
                ...
                return Response(
                    {"detail": f"{user.username}, you email is successfully verified."}
                )
        except Exception as e:
            return Response(
                {"error": str(e), "response": response},
                status=status.HTTP_400_BAD_REQUEST,
            )

        # If user instance returned is of type - None
        return Response(
            {"error": "Nothing was returned"}, status=status.HTTP_400_BAD_REQUEST
        )
...
class CustomTokenObtainPairView(TokenObtainPairView):
    ...
    def post(self, request: Request, *args, **kwargs) -> Response:
        ...
        if not user.check_password(password):
            return Response(
                {"error": "Error getting user details", "message": "Invalid Password"},
                status=status.HTTP_400_BAD_REQUEST,
            )
        ...
        if response.status_code == status.HTTP_200_OK:
            ...
            if not user.email_verified:
                ...
                link = send_email(
                    request=request, user=user, mail_type="verify_on_login"
                )
                return Response(
                    {
...
class SignUpview(generics.CreateAPIView):
    ...
    def create(self, request, *args, **kwargs):
        try:
            ...
            response = requests.post(
                url=f"https://{request.get_host()}/{os.getenv("API_VERSION")}mail/user-auth/",
                params={
                    "mail_type": "signup",
...
class UserPasswordResetRequestView(views.APIView):
    ...
    def post(self, request: Request, *args, **kwargs):
        ...
        return Response(
            {
                "message": "Kindly Check your mail to access the password reset link",
                "link": link,
            },
...
class VerifyPasswordResetView(generics.GenericAPIView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        if isinstance(user, User):
            return Response(
                {
                    "message": f"{user.username}, the link has been verified you can not reset your password.",
                    "reset link": reverse(
                        "userauth:new-password", kwargs={"safe": kwargs["safe"]}
                    ),
                },
                status=status.HTTP_200_OK,
            )
...
class UserPasswordResetConfirmView(generics.GenericAPIView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        return Response(
            {"message": "User password successfully updated"}, status=status.HTTP_200_OK
        )
